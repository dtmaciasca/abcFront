...
export class DetalleEventoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.registerForm = new FormGroup({
      ...
      esPresencial:  new FormControl({value: this.evento.esPresencial ? 'True': 'False', disabled: this.edit}, Validators.required),
    });
  ...
  editar() {
    ...
    const esPresencial = this.registerForm.get('esPresencial').value == ''?this.evento.esPresencial: this.registerForm.get('esPresencial').value;
    const user = '1';
